...
const userProfile = async (req, res, next) => {
    try {
        ...
        var userDisability = await db.disability.findAll({
            where: {
                id: {
            ...
            where: {
                [Op.or]: [
                    ...
                    },
                    {
                        active: true,
                        otherDisabilities: {
                            [Op.overlap]: req.cookies.User.Disability,
                        },
                        isOther: true,
                    },
                ],
...
const yourBooks = async (req, res, nest) => {
    try {
        var { limit, page } = req.query;
        var MyBooks = await db.books.findAndCountAll({
            limit: limit,
            offset: (parseInt(page) - 1) * page,
            where: {
                [Op.or]: [
            ...
            include: [{ model: db.disability, as: "booksDisability" }],
        });
...
const yourGide = async (req, res, nest) => {
    try {
        var { limit, page } = req.query;
        var yourGide = await db.guideLines.findAndCountAll({
            limit: limit,
            offset: (parseInt(page) - 1) * page,
            where: {
                [Op.or]: [
            ...
            include: [{ model: db.disability, as: "guideLinesDisability" }],
        });
...
const ExamVoice = async (req, res, nest) => {
    try {
        var { limit, page } = req.query;
        var ExamVoice = await db.soundExam.findAndCountAll({
            limit: limit,
            offset: (parseInt(page) - 1) * page,
            where: {
                [Op.or]: [
            ...
            include: [{ model: db.disability, as: "soundDisability" }],
        });
...
const enterExam = async (req, res, nest) => {
    try {
        ...
        if (TestResult && TestResult.success == null) {
            ...
            message =
                "لقد دخلت هذا الامتحان واجتزته بالفعل انتظر الاختبارات الاخري";
        } else if (TestResult && !TestResult.success) {
